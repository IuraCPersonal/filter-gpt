import {
  DEFAULT_STORAGE_DATA,
  ISSUES_STORAGE_KEY,
  type StorageData,
} from "@/config/constants";
import type { Issue } from "@/services/issues";

export const storage = {
  async getIssues(): Promise<StorageData> {
    try {
      const result = await chrome.storage.local.get(ISSUES_STORAGE_KEY);
      return (
        (result[ISSUES_STORAGE_KEY] as StorageData | null) ||
        DEFAULT_STORAGE_DATA
      );
    } catch (error) {
      console.error("Error getting issues from storage", error);
      return DEFAULT_STORAGE_DATA;
    }
  },

  async addIssue(value: string, context?: string) {
    const issues = await this.getIssues();

    const existingIssue = issues?.activeIssues.find(
      (issue) => issue.value === value
    );
    if (existingIssue) return;

    const newIssue: Issue = {
      id: Math.random().toString(36).substring(2, 15),
      value,
      detectedAt: Date.now(),
      context,
    };

    issues?.activeIssues.push(newIssue);
    issues?.historyIssues.push(newIssue);

    await this.setIssues(issues);
  },

  async setIssues(issues: StorageData) {
    try {
      await chrome.storage.local.set({ [ISSUES_STORAGE_KEY]: issues });
    } catch (error) {
      console.error("Error setting issues to storage", error);
    }
  },
};
